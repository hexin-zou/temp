...
public class ExcelUtil {
     ...
     * @param response     响应体
     */
    public static <T> void exportTemplateMultiSheet(List<Map<String, T>> data, String filename, String templatePath, HttpServletResponse response) {
        try {
            resetResponse(filename, response);
            ServletOutputStream os = response.getOutputStream();
     ...
     * @param os           输出流
     */
    public static <T> void exportTemplateMultiSheet(List<Map<String, T>> data, String templatePath, OutputStream os) {
        ClassPathResource templateResource = new ClassPathResource(templatePath);
        ExcelWriter excelWriter = EasyExcel.write(os)
            .withTemplate(templateResource.getStream())
            .autoCloseStream(false)
        ...
        for (int i = 0; i < data.size(); i++) {
            ...
            for (Map.Entry<String, ?> map : data.get(i).entrySet()) {
                // 设置列表后续还有数据
                FillConfig fillConfig = FillConfig.builder().forceNewRow(Boolean.TRUE).build();
                if (map.getValue() instanceof Collection) {
                    // 多表导出必须使用 FillWrapper
