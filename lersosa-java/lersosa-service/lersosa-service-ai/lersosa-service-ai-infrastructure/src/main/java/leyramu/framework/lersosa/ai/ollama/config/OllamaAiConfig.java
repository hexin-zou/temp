...
package leyramu.framework.lersosa.ai.ollama.config;

import leyramu.framework.lersosa.ai.ollama.tool.OllamaTools;
...
@AiProviderPrefix("ollama")
@ConditionalOnProperty(name = "spring.ai.ollama.enable", havingValue = "true")
public class OllamaAiConfig extends BaseAiConfig<OllamaChatModel, OllamaEmbeddingModel, OllamaTools> {
     ...
     */
    public OllamaAiConfig(OllamaChatModel chatModel, OllamaEmbeddingModel embeddingModel) {
     ...
     */
    @Override
    @Bean("ingestTermOfServiceToOllamaVectorStore")
    public CommandLineRunner ingestTermsOfServiceToVectorStore(
        @Qualifier("ollamaVectorStore") VectorStore vectorStore, ResourcePatternResolver resourcePatternResolver) {
    ...
    @ConditionalOnBean(OllamaTools.class)
    public ChatClient chatClient(@Qualifier("ollamaChatMemory") ChatMemory chatMemory, OllamaTools tool, ToolCallbackProvider mcpTools) {
        return super.chatClient(chatMemory, tool, mcpTools);
    }
}
