...
public class RecorderRemoveCmdExe {

    /**
     * 脉冲星记录映射器.
     */
    private final RecorderMapper recorderMapper;
    ...
    @GlobalTransactional(name = "deleteMarkAndRecorder", rollbackFor = Exception.class)
    public Boolean deleteByIds(Collection<Long> ids, Boolean isValid) {
        recorderMapper.selectByIds(ids).forEach(recorderE -> markMapper.updateFlagById(2, recorderE.getId()));
        if (recorderMapper.deleteByIds(ids) > 0) {
            return markMapper.deleteByIds(ids) > 0;
        }
        return false;
    }
}
