...
<template>
  <div class="p-2">
    ...
    <el-card shadow="hover">
      <template #header>
        <el-row :gutter="10" class="mb8">
          <el-col :span="1.5">
            <el-button v-hasPermi="['pulsar:recorder:add']" type="primary" plain icon="Plus" @click="handleAdd">新增 </el-button>
          </el-col>
          <el-col :span="1.5">
            <el-button v-hasPermi="['pulsar:recorder:edit']" type="success" plain icon="Edit" :disabled="single" @click="handleUpdate()"
              >修改
            </el-button>
          </el-col>
          <el-col :span="1.5">
            <el-button v-hasPermi="['pulsar:recorder:remove']" type="danger" plain icon="Delete" :disabled="multiple" @click="handleDelete()"
              >删除
            </el-button>
          </el-col>
          <el-col :span="1.5">
            <el-button v-hasPermi="['pulsar:recorder:export']" type="warning" plain icon="Download" @click="handleExport">导出 </el-button>
          </el-col>
          <right-toolbar v-model:show-search="showSearch" @query-table="getList"></right-toolbar>
        </el-row>
      </template>
      <el-empty v-if="total === 0" description="暂无数据" style="margin-top: 10%">
        <el-button v-hasPermi="['pulsar:recorder:list']" type="primary" class="submit" @click="getList"> 刷新页面 </el-button>
      </el-empty>
      <el-table v-else v-loading="loading" :data="pulsarList" @selection-change="handleSelectionChange">
        ...
        <el-table-column label="发现者" align="center" prop="discoverer" />
        <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
          <template #default="scope">
            <el-tooltip content="修改" placement="top">
              <el-button v-hasPermi="['pulsar:recorder:edit']" link type="primary" icon="Edit" @click="handleUpdate(scope.row)"></el-button>
            </el-tooltip>
            <el-tooltip content="删除" placement="top">
              <el-button v-hasPermi="['pulsar:recorder:remove']" link type="primary" icon="Delete" @click="handleDelete(scope.row)"></el-button>
            </el-tooltip>
          </template>
        </el-table-column>
      </el-table>
    ...
    <el-dialog v-model="dialog.visible" :title="dialog.title" width="500px" append-to-body>
      <el-form ref="RecorderFormRef" :model="form" :rules="rules" label-width="80px">
        <el-form-item label="脉冲星名字" prop="name">
        ...
        <el-form-item label="发现者" prop="discoverer">
          <el-input v-model="form.discoverer" placeholder="请输入发现者" />
        </el-form-item>
      </el-form>
      <template #footer>
...
import { listPulsar, getPulsar, delPulsar, addPulsar, updatePulsar } from '@/api/pulsar/recorder';
import { ref, onMounted, onActivated } from 'vue';
import { RecorderVo, RecorderQuery, RecorderForm } from '@/api/pulsar/recorder/types';

const { proxy } = getCurrentInstance() as ComponentInternalInstance;
const pulsarList = ref<RecorderVo[]>([]);
...
const RecorderFormRef = ref<ElFormInstance>();
const dialog = reactive<DialogOption>({
  visible: false,
  title: ''
});

const initFormData: RecorderForm = {
  id: undefined,
  name: undefined,
  period: undefined,
...
const data = reactive<PageData<RecorderForm, RecorderQuery>>({
  form: { ...initFormData },
  queryParams: {
    pageNum: 1,
    pageSize: 10,
  ...
  rules: {
    ...
    discoverer: [{ required: true, message: '发现者不能为空', trigger: 'blur' }]
  }
});
...
const reset = () => {
  ...
  RecorderFormRef.value?.resetFields();
};
...
/** 多选框选中数据 */
const handleSelectionChange = (selection: RecorderVo[]) => {
...
/** 修改按钮操作 */
const handleUpdate = async (row?: RecorderVo) => {
  reset();
...
/** 提交按钮 */
const submitForm = () => {
  RecorderFormRef.value?.validate(async (valid: boolean) => {
    if (valid) {
...
/** 删除按钮操作 */
const handleDelete = async (row?: RecorderVo) => {
